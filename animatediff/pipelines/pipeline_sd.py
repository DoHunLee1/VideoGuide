...
@dataclass
class SubStablePipelineOutput(BaseOutput):
    image: Union[torch.Tensor, np.ndarray]
    intermediate: Union[torch.Tensor, None]

class SubStableDiffusionPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        init_t = torch.Tensor([1000])
        init_t = init_t.to(noise.device)
        noise_input = torch.cat([noise] * 2)
        v_pred = self.sub_unet(noise_input, init_t, encoder_hidden_states=text_embeddings).sample
        v_pred_uncond, v_pred_cond = v_pred.chunk(2)
        v_pred_cfg = v_pred_uncond + guidance_scale * (v_pred_cond - v_pred_uncond)
                
        intermediate = self.decode_latents(-1*v_pred_cond)
        intermediate = torch.from_numpy(intermediate)
        intermediate_cfg = self.decode_latents(-1 * v_pred_cfg)
        intermediate_cfg = torch.from_numpy(intermediate_cfg)
        intermediate = torch.cat([intermediate, intermediate_cfg])
        

        final_timestep = timesteps[0]
        alpha_T = self.scheduler.alphas_cumprod[final_timestep]
        latents = -1 * alpha_T.sqrt() * v_pred_cond + (1 - alpha_T).sqrt() * noise
        ...
        return SubStablePipelineOutput(image=image, intermediate=intermediate)