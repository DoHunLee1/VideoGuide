...
class SparseControlNetModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.controlnet_down_blocks.append(controlnet_block) # zero_module(Inflatedconv3d)
    ...
    def forward(
        ...
        encoder_hidden_states: torch.Tensor, # Prompt
        ...
        if self.set_noisy_sample_input_to_zero:
            sample = torch.zeros_like(sample).to(sample.device) # sample은 받지 않음
        ...
        print("2: ", controlnet_cond.shape)
        controlnet_cond = self.controlnet_cond_embedding(controlnet_cond)
        print("3: ", controlnet_cond.shape)
